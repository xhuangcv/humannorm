...
class DirectionConfig:
    ...
    condition: Callable[
        [Float[Tensor, "B"], Float[Tensor, "B"], Float[Tensor, "B"], Int[Tensor, "B"]],
        Float[Tensor, "B"],
    ]
...
class PromptProcessor(BaseObject):
    ...
    def configure(self) -> None:
        ...
        if self.cfg.human_part_prompt:
            self.directions = [
                DirectionConfig(
                    "front_full",
                    lambda s: f"{s}, full body, front view",
                    lambda s: s,
                    lambda ele, azi, dis, part: ((
                        shift_azimuth_deg(azi) > -self.cfg.front_threshold
                    )
                    & (shift_azimuth_deg(azi) < self.cfg.front_threshold)) & (part == 0),
                ),
                DirectionConfig(
                    "back_full",
                    lambda s: f"{s}, full body, back view",
                    lambda s: s,
                    lambda ele, azi, dis, part: ((
                        shift_azimuth_deg(azi) > 180 - self.cfg.back_threshold
                    )
                    | (shift_azimuth_deg(azi) < -180 + self.cfg.back_threshold)) & (part == 0),
                ),
                DirectionConfig(
                    "left_full",
                    lambda s: f"{s}, full body, left side view",
                    lambda s: s,
                    lambda ele, azi, dis, part: ((
                        shift_azimuth_deg(azi) >= self.cfg.front_threshold
                    )
                ...
                DirectionConfig(
                    "right_full",
                    lambda s: f"{s}, full body, right side view",
                    lambda s: s,
                    lambda ele, azi, dis, part: ((
                        shift_azimuth_deg(azi) >= -180 + self.cfg.back_threshold
                    )
                ...
                DirectionConfig(
                    "front_head",
                    lambda s: f"{s}, only head, front view",
                    lambda s: s,
                    lambda ele, azi, dis, part: ((
                        shift_azimuth_deg(azi) > -self.cfg.front_threshold
                    )
                    & (shift_azimuth_deg(azi) < self.cfg.front_threshold)) & (part == 1),
                ),
                DirectionConfig(
                    "back_head",
                    lambda s: f"{s}, only head, back view",
                    lambda s: s,
                    lambda ele, azi, dis, part: ((
                        shift_azimuth_deg(azi) > 180 - self.cfg.back_threshold
                    )
                    | (shift_azimuth_deg(azi) < -180 + self.cfg.back_threshold)) & (part == 1),
                ),
                DirectionConfig(
                    "left_head",
                    lambda s: f"{s}, only head, left side view",
                    lambda s: s,
                    lambda ele, azi, dis, part: ((
                        shift_azimuth_deg(azi) >= self.cfg.front_threshold
                    )
                ...
                DirectionConfig(
                    "right_head",
                    lambda s: f"{s}, only head, right side view",
                    lambda s: s,
                    lambda ele, azi, dis, part: ((
                        shift_azimuth_deg(azi) >= -180 + self.cfg.back_threshold
                    )
                ...
                DirectionConfig(
                    "front_upper",
                    lambda s: f"{s}, upper body, front view",
                    lambda s: s,
                    lambda ele, azi, dis, part: ((
                        shift_azimuth_deg(azi) > -self.cfg.front_threshold
                    )
                    & (shift_azimuth_deg(azi) < self.cfg.front_threshold)) & (part == 2),
                ),
                DirectionConfig(
                    "back_upper",
                    lambda s: f"{s}, upper body, back view",
                    lambda s: s,
                    lambda ele, azi, dis, part: ((
                        shift_azimuth_deg(azi) > 180 - self.cfg.back_threshold
                    )
                    | (shift_azimuth_deg(azi) < -180 + self.cfg.back_threshold)) & (part == 2),
                ),
                DirectionConfig(
                    "left_upper",
                    lambda s: f"{s}, upper body, left side view",
                    lambda s: s,
                    lambda ele, azi, dis, part: ((
                        shift_azimuth_deg(azi) >= self.cfg.front_threshold
                    )
                ...
                DirectionConfig(
                    "right_upper",
                    lambda s: f"{s}, upper body, right side view",
                    lambda s: s,
                    lambda ele, azi, dis, part: ((
                        shift_azimuth_deg(azi) >= -180 + self.cfg.back_threshold
                    )
                ...
                DirectionConfig(
                    "front_lower",
                    lambda s: f"{s}, lower body, front view",
                    lambda s: s,
                    lambda ele, azi, dis, part: ((
                        shift_azimuth_deg(azi) > -self.cfg.front_threshold
                    )
                    & (shift_azimuth_deg(azi) < self.cfg.front_threshold)) & (part == 3),
                ),
                DirectionConfig(
                    "back_lower",
                    lambda s: f"{s}, lower body, back view",
                    lambda s: s,
                    lambda ele, azi, dis, part: ((
                        shift_azimuth_deg(azi) > 180 - self.cfg.back_threshold
                    )
                    | (shift_azimuth_deg(azi) < -180 + self.cfg.back_threshold)) & (part == 3),
                ),
                DirectionConfig(
                    "left_lower",
                    lambda s: f"{s}, lower body, left side view",
                    lambda s: s,
                    lambda ele, azi, dis, part: ((
                        shift_azimuth_deg(azi) >= self.cfg.front_threshold
                    )
                ...
                DirectionConfig(
                    "right_lower",
                    lambda s: f"{s}, lower body, right side view",
                    lambda s: s,
                    lambda ele, azi, dis, part: ((
                        shift_azimuth_deg(azi) >= -180 + self.cfg.back_threshold
                    )
