...
class StableDiffusionVSDGuidance(BaseModule):
    ...
    def compute_grad_vsd(
        ...
        noise_map: Float[Tensor, "B 4 64 64"],
    ):
        B = latents.shape[0]
        ...
        with torch.no_grad():
            ...
            # add noise
            noise = noise_map if noise_map is not None else torch.randn_like(latents)
    ...
    def forward(
        ...
        noise_map: Float[Tensor, "B 4 64 64"]=None,
        **kwargs,
    ):
        ...
        grad = self.compute_grad_vsd(
            latents, text_embeddings_vd, text_embeddings, camera_condition, noise_map
        )
