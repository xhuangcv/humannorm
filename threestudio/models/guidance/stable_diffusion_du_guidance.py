...
@threestudio.register("stable-diffusion-du-guidance")
class ControlNetGuidance(BaseObject):
    ...
    def configure(self) -> None:
        ...
        if self.cfg.control_type == 'normal' or self.cfg.control_type == 'input_normal':
            controlnet_name_or_path: str = "/home/xinhuang/.cache/huggingface/hub/models--lllyasviel--control_v11p_sd15_normalbae/snapshots/cb7296e6587a219068e9d65864e38729cd862aa8"
        elif self.cfg.control_type == 'canny':
            controlnet_name_or_path: str = "/home/xinhuang/.cache/huggingface/hub/models--lllyasviel--control_v11p_sd15_canny/snapshots/115a470d547982438f70198e353a921996e2e819"
    ...
    def edit_latents(
        ...
        print("diffusion steps: ", t.item() // 50 + 1)
        # self.scheduler.config.num_train_timesteps = t.item()
        # self.scheduler.set_timesteps(self.cfg.diffusion_steps)
        with torch.no_grad():
            # add noise
    ...
    def __call__(
        ...
        **kwargs,
        ...
        text_embeddings = prompt_utils.get_text_embeddings(elevation, azimuth, camera_distances, True)
        ...
        if self.cfg.use_sds:
            ...
            temp = (edit_images[0].permute(1,2,0).detach().cpu().numpy() * 255).astype(np.uint8)
            cv2.imwrite(".threestudio_cache/test.jpg", temp[:, :, ::-1])

            # loss_sds = 0.5 * F.mse_loss(latents, edit_latents.detach(), reduction="sum") / batch_size
